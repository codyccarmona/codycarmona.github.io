...
const GameBoard = (() =>{
    ...
    const checkDiagnolWin = async () =>{
        ...
        if((_board[0] && _board[_properties.bottomRightCorner])&& 
            _board[0] == _board[_properties.bottomRightCorner]){
                currPlayerId = _board[0];
                for(let i = _properties.leftToRightStep; i < _board.length; i + _properties.leftToRightStep){
        ...
        else if((_board[_properties.topRightCorner] && _board[_properties.bottomLeftCorner])&& 
                (_board[0] == _board[_properties.bottomRightCorner])){
                    currPlayerId = _board[_properties.topRightCorner];
                    for(let i = (_properties.topRightCorner + _properties.rightToLeftStep); i < _board.length; i + _properties.rightToLeftStep){
                        if(_board[i] != currPlayerId){
                            currPlayerId = null;
                            break;
                        }
